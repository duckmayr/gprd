...
#'
#' @export
plot.gprd <- function(x, from = NULL, to = NULL,
    ...
    if ( x$estimator == "local" ) {
        ...
        preds  <- .gprd_predict(f$training_outcomes,
                                f$training_input,
                                cbind(test_points, test_points > x$cutoff),
    ...
    if ( x$estimator == "local" ) {
        ...
        points(left_inputs,  left_outcomes,  pch = 19, col = data_color)
        points(right_inputs, right_outcomes, pch = 19, col = data_color)
        ...
        lim <- range(c(low, high, output), na.rm = TRUE)
        plot(x = test_points, y = pred_mean, ylim = lim, type = "n", ...,
             main = main_title, xlab = "", ylab = "", xaxt = "n", yaxt = "n")
        points(inputs, output, pch = 19, col = data_color)
        polygon(x = c(test_points, rev(test_points)), y = c(low, rev(high)),
                border = NA, col = ci_color)
        lines(test_points, pred_mean, col = line_color)
    }
